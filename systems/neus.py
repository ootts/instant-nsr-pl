...
from pytorch_lightning.utilities.distributed import rank_zero_info, rank_zero_debug
...
class NeuSSystem(BaseSystem):
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.config.dataset.name == "oneposev2":
            outdir = f"/home/linghao/Datasets/onepose_datasets/lowtexture_test_data/{self.config.dataset.scene}/{self.dataset.subseq}/vr_depth"
            os.makedirs(outdir, exist_ok=True)
            out_path = osp.join(outdir, batch['imgid'][0] + '.png')
            cv2.imwrite(out_path, (out['depth'].reshape(H, W).cpu().numpy().astype(np.float32) * 1000).astype(np.uint16))
        return {
            'psnr': psnr,
            'index': batch['index']
        }
